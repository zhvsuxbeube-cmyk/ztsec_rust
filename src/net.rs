use std::{io::{BufRead, BufReader, ErrorKind, Write}, net::TcpStream, thread, time::{Duration, Instant}};
    ...
    loop {
        ...
        match TcpStream::connect((ip, port)) {
            Ok(mut stream) => {
                ...
                if send(&mut stream, &format!("{}{}", text::HELLO, fp)).is_ok() && send(&mut stream, &format!("{}{}", text::DATA, data)).is_ok() {
                    println!("{}", text::CONNECTED);
                    if session(&mut stream, ip, &fp) { return; }
                }
            }
            Err(_) => {}
        }
...
fn session(stream: &mut TcpStream, ip: &str, fp: &str) -> bool {
    ...
    loop {
        match line(&mut reader) {
            Ok(Some(value)) if value == text::HB => { let _ = send(stream, text::PONG); }
            ...
            Ok(Some(value)) if value.starts_with(text::CMD) => {
                ...
                match cmd.as_str() {
                    text::RECONNECT => { let _ = send(stream, &format!("{}{}", text::ACK, cmd)); return false; }
                    text::CLOSE => { let _ = send(stream, &format!("{}{}", text::ACK, cmd)); println!("{}", text::CLOSED); return true; }
            ...
            Err(e) if matches!(e.kind(), ErrorKind::TimedOut | ErrorKind::WouldBlock) => { let _ = send(stream, text::HB); }
            Err(_) => return false,
        }
    }
}
...
fn line(r: &mut BufReader<TcpStream>) -> std::io::Result<Option<String>> {
    ...
    Ok(match r.read_line(&mut s)? { 0 => None, _ => Some(s.trim_end_matches(['\r','\n']).to_owned()) })
}

fn send(stream: &mut TcpStream, s: &str) -> std::io::Result<()> { stream.write_all(s.as_bytes())?; stream.write_all(b"\n") }
